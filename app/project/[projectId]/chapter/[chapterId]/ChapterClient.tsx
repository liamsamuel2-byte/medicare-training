...
interface Chapter {
  ...
  videoUrl: string | null; videoDuration: number | null;
...
export default function ChapterClient({
  ...
  const [passed, setPassed] = useState(existingPassed);
  ...
  async function markNoQuizComplete() {
    await fetch("/api/chapter-complete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chapterId: chapter.id, projectId }),
    });
    setPassed(true);
  }

  useEffect(() => {
    if (noQuiz && mediaViewed && !passed) {
      markNoQuizComplete();
    }
  }, [noQuiz, mediaViewed]);
  ...
  async function handleSubmitQuiz() {
    ...
    const res = await fetch("/api/quiz-submit", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        chapterId: chapter.id, projectId,
        responses: Object.entries(selected).map(([questionId, answerId]) => ({ questionId, answerId })),
      }),
    });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        <div>
          ...
          {chapter.description && <p className="text-gray-500 mt-1">{chapter.description}</p>}
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            {!mediaViewed && (
              <div className="p-4 border-t border-gray-100">
                <button
                  onClick={() => { setMediaViewed(true); setShowQuiz(true); }}
                  className="bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-800 transition"
                >
                  I've reviewed this material → Continue to Quiz
                </button>
              </div>
            )}
          </div>
        ...
        {!chapter.videoUrl && !chapter.imageUrl && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-800 text-sm">
            No media has been uploaded for this chapter yet.
          </div>
        )}
        ...
        {showQuiz && !noQuiz && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            {submitted && score !== null && score < PASS_RATE && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center justify-between">
                <div>
                  <p className="text-red-700 font-semibold text-lg">{Math.round(score)}% — Not quite!</p>
                  <p className="text-red-600 text-sm mt-0.5">You need {PASS_RATE}% to pass. Review the answers below and try again.</p>
                </div>
                <button
            ...
            {chapter.questions.map((q, qi) => (
              <div key={q.id} className="space-y-3">
                ...
                {q.imageUrl && (
                  <Image src={q.imageUrl} alt="Question image" width={600} height={400} className="rounded-lg max-h-64 object-contain" />
                )}
                <div className="space-y-2">
                  {q.answers.map((a) => {
                    const isSelected = selected[q.id] === a.id;
                    const showResult = submitted;
                    let classes = "flex items-center gap-3 px-4 py-3 rounded-lg border cursor-pointer transition text-sm ";
                    if (showResult) {
                      if (a.isCorrect) classes += "border-green-400 bg-green-50 text-green-800";
                      else if (isSelected && !a.isCorrect) classes += "border-red-400 bg-red-50 text-red-800";
                    ...
                    return (
                      <label key={a.id} className={classes}>
                        <input
                          type="radio" name={q.id} value={a.id}
                          disabled={submitted}
                          checked={isSelected}
                        ...
                        />
                        {a.text}
                        {showResult && a.isCorrect && <CheckCircle size={16} className="ml-auto text-green-600" />}
                      </label>
                    );
                  })}
                </div>
